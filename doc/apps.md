...
> 通用操作：**长按任意界面顶部标题栏文字可截屏**，图片以 BMP 格式保存到 SD 卡 `/screenshot/` 目录。
...
- 点击右上角设置按钮进入设置面板：
  ...
  - **根音音名**：13 档可调
  - **钢琴音色**：16 种 SF2 乐器音色
  - **延音开关**：开启后抬手每个音保留约 1.8 秒自然尾音（类 Cave 空灵感，有界不混叠）
  - **起始八度**：C0 ~ C6 滚轮选择
- 点击录制按钮可录制演奏
...
- **轨道**：左侧 8 条轨道点击选中（点击即试听该轨当前音色）；参数面板可切换 23 种 GM 打击乐音色（切换/恢复默认即试听）、静音 / 独奏、力度、触发概率、随机时值（人性化）
...
- 点击右上角设置按钮：
  - **BPM 范围**：20 ~ 300
  - **拍号**：分子 1~16（每小节拍数，决定重拍循环与节拍灯数）/ 分母 1/2/4/8/16（拍单位，如 4 = 四分音符为一拍、8 = 八分音符为一拍）
