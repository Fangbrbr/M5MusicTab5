...
public:
    ...
    // PATCH(P1): bool begin() 声明已剔除（TLV 状态持久化不使用）
    ...
    SF2Parser& parser;
    // PATCH(P1): begin/loadSynthState/saveSynthState 声明已剔除（TLV 状态持久化不使用）
    const String& getCurrentSf2Path() const { return currentSf2Path; }

    // PATCH(P10): 遥测用活跃 voice 计数（引擎适配层调试快照）
    int activeVoiceCount() const { int n = 0; for (int i = 0; i < MAX_VOICES; ++i) if (voices[i].active) ++n; return n; }
