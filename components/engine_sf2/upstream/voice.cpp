...
// PATCH(P2): Voice::usage 定义从 .ino 移入（上游定义在应用入口，组件化后须随库走）
int Voice::usage = 0;
...
void Voice::prepareStart(uint8_t ch, uint8_t note_, uint8_t vel, const Zone& z, ChannelState* chan) {
    ...
    // PATCH(P11): 上游默认开启 ENABLE_CH_FILTER_M 故此处未加守卫，关闭后编译失败
    ...
    // PATCH(P12): 上游笔误 ch->delaySend（ch 是 uint8_t 通道号），Arduino 高日志级别下被编译剔除未暴露
    ESP_LOGD(TAG, "ch=%d reverb=%.5f chorus=%.5f delay=%.5f", channel, reverbAmount, chorusAmount, chan->delaySend);
...
float __attribute__((hot,always_inline)) IRAM_ATTR Voice::nextSample() {
    ...
    // PATCH(P9): 修复幅度量双重叠加——velocityVolume/modVolume/modExpression 在
    // renderLRBlock 的 volL/volR 已按块应用，此处不再每样本重复乘（上游为平方化响应）
    float val = smp * env;
...
void Voice::updateScore() {
    ...
    // PATCH(P13): 按住中的音符（含弦乐长音）评分加权，抢音时优先牺牲
    // 已释放/低电平的 voice，避免持续长音被新音符挤掉造成“长音变短促”
    if (noteHeld) {
        score *= 1.5f;
    }
}
