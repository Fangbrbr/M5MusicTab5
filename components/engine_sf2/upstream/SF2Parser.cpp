...
// PATCH(P7): preset/instrument 名按 20 字节定长拷贝，SF2 name 字段不保证 NUL 结尾
static inline String sf2FixedName(const char raw[20]) {
    size_t n = 0;
    while (n < 20 && raw[n] != '\0') {
        n++;
    }
...
bool SF2Parser::parsePDTA() {
    ...
    for (size_t i = 0; i + 1 < phdrs.size(); ++i) {
        SF2Preset preset;
        preset.name = sf2FixedName(phdrs[i].name);  // PATCH(P7): 定长拷贝防过读
    ...
    for (size_t i = 0; i + 1 < insts.size(); ++i) {
        SF2Instrument inst;
        inst.name = sf2FixedName(insts[i].name);  // PATCH(P7): 定长拷贝防过读
...
void SF2Parser::applyGenerators(const sf2_vector<Generator>& gens, Zone& zone) {
    for (const auto& g : gens) {
        ...
        switch (op) {
            ...
            case GeneratorOperator::ModLfoFreq:
                ...
                break;
            // PATCH(P5): 补上 SF2 规范的 InitialAttenuation（centibels → 线性衰减），
            // 上游缺失导致音色响度映射失真
            case GeneratorOperator::InitialAttenuation:
    ...
    // PATCH(P6): 移除"发送量为 0 强制改 1.0"的上游私货（非 SF2 语义，
    // 效果发送保持解析值，默认 0 = 不发）
}
...
bool SF2Parser::loadSampleDataToMemory() {
    ...
    for (size_t i = 0; i < samples.size(); ++i) {
        ...
        if (m_progress_cb != nullptr) {
            m_progress_cb((int)((i + 1) * 100 / samples.size()), m_progress_user);
        }
        ...
        // PATCH(P16): IDF 5.4 弃用 heap_caps_aligned_alloc/free，换通用接口；
        // 采样仅需 2 字节对齐，heap_caps_malloc 默认对齐已满足
...
void SF2Parser::clear() {
    ...
    for (size_t i = 0; i < samples.size(); ++i) {
        ...
        for (size_t j = 0; j < i; ++j) {
            if (samples[j].data == samples[i].data) {
                alias = true;
                break;
            }
        }
