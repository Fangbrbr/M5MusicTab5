...
public:
    ...
    // PATCH(P3): 采样加载进度回调（0-100），供 boot 进度条；上游无此机制
    typedef void (*SF2ProgressCb)(int percent, void *user_data);
    void setProgressCallback(SF2ProgressCb cb, void *user_data) {
        m_progress_cb = cb;
        m_progress_user = user_data;
    }
...
private:
    ...
    /* PATCH(P3): 进度回调成员 */
    SF2ProgressCb m_progress_cb = nullptr;
