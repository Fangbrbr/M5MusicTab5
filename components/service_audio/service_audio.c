...
esp_err_t service_audio_init(void)
{
    ...
    esp_err_t ret_route = bsp_feature_enable(BSP_FEATURE_SPEAKER, !headphone_connected);
    if (ret_route != ESP_OK) {
...
static void service_audio_headphone_route_update(void)
{
    ...
    esp_err_t ret = bsp_feature_enable(BSP_FEATURE_SPEAKER, !headphone_connected);
    if (ret != ESP_OK) {
