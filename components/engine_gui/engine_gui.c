...
/* 逻辑分辨率 1280×720，flush 时旋转 90° 写入物理 720×1280 帧缓冲 */
#define UI_LOGICAL_H_RES    1280
#define UI_LOGICAL_V_RES    720
...
#define DSI_FB_SIZE         ((size_t)BSP_LCD_H_RES * BSP_LCD_V_RES * sizeof(uint16_t))
...
static void engine_gui_brightness_settle(void)
{
    ...
    bsp_display_brightness_set((uint8_t)pending);
                                    ...
                                    int16_t *log_x, int16_t *log_y)
{
    if (s_disp_inverted) {
        ...
        *log_x = (int16_t)raw_y;
        *log_y = (int16_t)(BSP_LCD_H_RES - 1 - (int)raw_x);
    } else {
        /* 90°：与 ROTATION_90 同式 */
        *log_x = (int16_t)((int)BSP_LCD_V_RES - 1 - (int)raw_y);
        *log_y = (int16_t)raw_x;
    }
}
...
static void engine_gui_refresh_status_bar(void)
{
    ...
    /* AI 就绪：小智 WebSocket Token 非空（设备已在 xiaozhi.me 完成绑定） */
    char xz_token[8] = {0};
    service_nvs_get_xz_ws_token(xz_token, sizeof(xz_token));
    if (xz_token[0] != '\0') {
        if (has_icon) {
            strncat(buf, " ", sizeof(buf) - strlen(buf) - 1);
        }
...
esp_err_t engine_gui_init(void)
{
    ...
    bsp_display_brightness_set(boot_brightness);
    lv_timer_handler();
...
#define AI_LED_HOME_X       620
#define AI_LED_HOME_Y       677
#define AI_LED_OFF_Y        760     /* 屏高 720 + LED 高 35，完全屏外 */
#define AI_LED_HOME_W       40
#define AI_LED_HOME_H       35
...
static void engine_gui_backlight_load_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_SCREEN_LOAD_START) {
        ...
        /* 熄屏期间被自动熄屏接管，不主动恢复 */
        if (!service_power_is_screen_off()) {
            bsp_display_brightness_set(s_backlight_saved_brightness);
        }
