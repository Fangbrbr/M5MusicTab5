...
/* Why: 无 RX8130CE 的板型由 CONFIG_BOARD_HAS_RTC 编译期门控，
 * 全部 API 以系统时钟（time/localtime/settimeofday）兜底，调用方零改动；
 * 闹钟依赖 RTC 硬件中断，无硬件时返回 NOT_SUPPORTED。 */
...
#endif /* CONFIG_BOARD_HAS_RTC */

extern "C" esp_err_t service_rtc_init(void)
{
#if !CONFIG_BOARD_HAS_RTC
    /* 系统时钟兜底路径仍需 TZ，保证 localtime 行为与有 RTC 板型一致 */
    setenv("TZ", "CST-8", 1);
    tzset();
    ESP_LOGW(TAG, "board has no RTC, fall back to system time");
    return ESP_OK;
#else
    if (s_rtc != NULL) {
        return ESP_OK;
...
#if !CONFIG_BOARD_HAS_RTC
    /* 无硬件 RTC：写入系统时钟，设置时间功能保持可用（掉电后丢失） */
    struct tm tmp = *tm;
    time_t t = mktime(&tmp);
    if (t == (time_t)-1) {
        return ESP_FAIL;
