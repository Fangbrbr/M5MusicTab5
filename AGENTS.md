...
- `check_widget_bindings.py`：校验 App 控件绑定与 EEZ 工程控件名一致性。
...
| `check_widget_bindings.py` | 校验 `WIDGET_BIND` 与 EEZ 控件一致性（构建期自动调用） |
...
- EEZ 工程中 `fileSystemPath` 必须保持 `/sys/src`；App 中的 `WIDGET_BIND` 名称必须与 EEZ 控件名完全一致，构建期由 `check_widget_bindings.py` 校验。
...
| **C6 提前上电** | `components/service_power/service_power.c:130-131` 中 `service_power_c6_early_enable()` 在调度器启动前操作 GPIO，不能调用 FreeRTOS API。 |
