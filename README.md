...
📺 **[点击观看项目演示视频](https://www.bilibili.com/video/BV1A7ua6WEH8)**
...
- **12 个内置 App** — 禅模式 / 练耳 / 和弦练习 / 五度圈 / 小钢琴 / 组鼓 / 播放器 / XY Pad / 节拍器 / AI 语音助手 / 时钟日历 / 趣味抽卡
- **SF2 采样合成器** — 基于 SoundFont 2 的真实乐器采样合成；支持从 SD 卡加载自定义音色（设置页一键切换，开机自动恢复上次选择），并针对大容量音源优化了复音性能
- **MP3 音乐播放** — 播放器 App 直接播放 SD 卡中的 .mp3（ID3 歌名显示、自动连播），中文文件名完整支持
- **全局 MIDI 总线** — 支持 USB Host、蓝牙、UART 多路 MIDI 输入，生产者/消费者解耦架构
- **演奏录制与回放** — 支持录制的 App 点击录制按钮开始、再点停止，你的灵感不再随风而逝；演奏以 .hmr（MIDI 事件流）存入 SD 卡，播放器 App 中一键回放
- **FTP 无线文件管理** — 设置页一键开启，免拔卡免接线，局域网内用 FileZilla / 资源管理器直接管理 SD 卡全部文件（传 MIDI 曲、取演奏录音、上传/更换音源文件等）
...
- 所有支持录制的 App（小钢琴、组鼓、XY Pad、禅模式、练耳）录制出的文件，都可以在「录音文件播放」模式查看并播放
...
设备内置 FTP 服务器，SD 卡整卡内容可通过局域网无线管理：给播放器传曲、取回演奏录音（.hmr）、上传/替换 SoundFont 音源，全程免拔卡、免接线。
