...
- **🇨🇳 中英双语界面** — 设置页一键切换，重启记忆
- **12 个内置 App** — 禅模式 / 练耳 / 和弦练习（含五度圈）/ 小钢琴 / 音序器 / 播放器 / XY Pad / 节拍器 / AI 语音助手 / 时钟日历 / 趣味抽卡 / 录音机
...
- **一键截屏** — 长按任意界面标题栏，当前画面存为 BMP 到 SD 卡
...
| `/wav/` | 录音机 `.wav` 录音文件 | 首次录音时创建 |
| `/sequencer/` | 音序器 pattern `.m5p` 文件 | 首次保存 pattern 时创建 |
| `/screenshot/` | 截屏 `.bmp` 文件（长按界面标题栏） | 首次截屏时创建 |
